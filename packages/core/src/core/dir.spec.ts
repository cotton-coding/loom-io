...
describe("Test Directory Service", () => {
  ...
  describe("with generated file", () => {
    ...
    describe("delete", () => {
      ...
      test("delete a non empty directory", async () => {
        ...
        await adapterHelper.createFile(
          joinPath(adapterHelper.last!, "to_delete/cotton.txt"),
        );
      ...
      test("delete a non empty directory recursive", async () => {
        ...
        await dir.create();
        await adapterHelper.createFile(
          joinPath(adapterHelper.last!, "to_delete/test_some_other"),
        );
    ...
    describe.concurrent("list method", () => {
      ...
      test("list with subDirectory", async () => {
        ...
        const testDir = getUniqSegmentsOfPath(testPaths, 2).map((p) =>
          addPrecedingSlash(p),
        );
    ...
    describe("files method", () => {
      test("files test returned amount", async () => {
        ...
        for (const f of files) {
          expect(f).toBeInstanceOf(LoomFile);
  ...
  describe("test symbols", () => {
    ...
    test("toStringTag", () => {
      const dir = new Directory(adapter, "/test/data");
      expect(Object.prototype.toString.call(dir)).toBe(
        "[object LoomDirectory]",
      );
    });
  });
});
